...
class CornersProblem(search.SearchProblem):
    ...
    class CornersState:
        """
        A state in the CornersProblem is represented by the current position of
        Pacman, and whether or not the four corners have been visited.
        """
        def __init__(self, position: Tuple[int, int], corners: List[bool]):
    ...
    def getSuccessors(self, state: Any):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                cost = 1
                successors.append((self.CornersState((nextx, nexty), corners), action, cost))
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def findPathToClosestDot(self, gameState: pacman.GameState):
        ...
        closetFood = None
        closetFoodDis = 999999
        for foodPos in food.asList():
            dis = util.manhattanDistance(startPosition, foodPos)
            if dis < closetFoodDis:
                closetFoodDis = dis
                closetFood = foodPos
        path = search.bfs(AnyFoodSearchProblem(gameState))
        return path
